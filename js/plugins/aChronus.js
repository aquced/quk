 ...
 * @author トリアコンタン 翻译：Zeldashu
 *
 * @param 月ごとの日数配列
 * @desc 设置每个月份的天数，用逗号分隔，数目随意。
 * @default 31,28,31,30,31,30,31,31,30,31,30,31
 *
 * @param 月名配列
 * @desc 月份名称，用逗号分隔，数目随意。
 * @default Jan.,Feb.,Mar.,Apr.,May.,Jun.,Jul.,Aug.,Sep.,Oct.,Nov.,Dec.
 *
 * @param 曜日配列
 * @desc 星期名称，用逗号分隔，数目随意。
 * @default (日),(月),(火),(水),(木),(金),(土)
 *
 * @param 自然時間加算
 * @type number
 * @desc 每一秒（下面"自然時間加算間隔"里指定的）增加的游戏时间（分钟为单位）。事件处理期间无效。
 * @default 5
 *
 * @param 自然時間加算間隔
 * @type number
 * @desc 游戏时间多少帧增加一次。60帧一秒。
 * @default 60
 *
 * @param 場所移動時間加算
 * @type number
 * @desc 场所移动时增加的时间（分钟为单位）
 * @default 30
 *
 * @param 戦闘時間加算(固定)
 * @type number
 * @desc 一次战斗增加的时间（分钟为单位）
 * @default 30
 *
 * @param 戦闘時間加算(ターン)
 * @type number
 * @desc 战斗每回合消耗的时间（分钟为单位）
 * @default 5
 *
 * @param 年のゲーム変数
 * @type variable
 ...
 * @param フォーマット時間の変数
 * @type variable
 * @desc 基于「フォーマット時間の計算式」此计算的结果将自动设置。
 * @default 0
 *
 * @param フォーマット時間の計算式
 * @desc 使用日期和时间格式的计算公式。
 * YYYY:年 MON:月名 MM:月 DD:日 等等(有关详细信息，请参阅帮助)
 * @default HH24 * 60 + MI
 *
 * @param 日時フォーマット1
 * @desc 显示在地图上的日期窗口第一行中的字符串。
 * YYYY:年 MON:月名 MM:月 DD:日 等等(有关详细信息，请参阅帮助)
 * @default YYYY年 MM月 DD日 DY
 *
 * @param 日時フォーマット2
 * @desc 显示在地图上的日期窗口第二行中的字符串。
 * YYYY:年 MON:月名 MM:月 DD:日 等等(有关详细信息，请参阅帮助)
 * @default AMHH時 MI分
 *
 * @param 日時フォーマット行間
 ...
 * @param 時計X座標
 * @type number
 * @desc 模拟时钟的X坐标。请指定图像的中心坐标。
 * @default 84
 *
 * @param 時計Y座標
 * @type number
 * @desc 模拟时钟的Y坐标。请指定图像的中心坐标。
 * @default 156
 *
 * @param イベント中時間経過
 * @desc 事件执行过程事件也会流逝。(ON/OFF)
 * @default false
 * @type boolean
 *
 * @help 这个插件可以在游戏中表达时间和天气的概念。
 * 随着时间推移，战斗、移动，色调和天气会发生实时变化。
 * 这些时间是可调的，并且在活动期间时间不再继续流逝。
 *
 * 此外，它还具有在游戏中反映真实时间的功能。
 * 当您启用设置时，现实时间将链接到游戏中。
 *
 * 您还可以记录日期和星期几，并自由设置星期几的编号和名称。
 * 根据格式，当前日期显示在屏幕的左上方。
 *
 * 日期格式可以使用以下内容：
 * YYYY:年 MON:月名 MM:月 DD:日 HH24:時(24) HH:時(12)
 * AM:午前 or 午後 MI:分 DY:星期 TZ 时区名称
 * DDALL:累计天数
 *
 * 您还可以通过准备符合标准的图像来显示模拟时钟。
 * 图像的显示位置可以自行调节。
 *
 * 图像规格如下。
 * ・表盘 : 任意大小的正方形图像
 * ・長針　 : 与表盘尺寸相同的图像，指向上(0)的针的图像
 * ・短針　 : 同上
 *
 * 
 * 这里有相应的参考素材，使用的话请注意其中的条约：
 ...
 *  (参数之间用半角空格分隔)
 *
 * C_ADD_TIME [分] : 时间经过（分钟）。
 * C_ADD_DAY [日] : 时间经过（天）。
 * C_SET_TIME [時] [分] : 更改为指定时间。
 * C_SET_DAY [年] [月] [日] : 更改为指定的日期。
 * C_STOP : 停止时间流逝
 * C_START : 启用时间流逝
 * C_SHOW : 显示日历
 * C_HIDE : 隐藏日历
 * C_DISABLE_TINT : 禁止根据时间段更改色调。
 * C_ENABLE_TINT : 启用根据时间段更改色调。
 * C_DISABLE_WEATHER : 禁止天气随时间变化。
 * C_ENABLE_WEATHER : 启用天气随时间的变化。
 * C_SET_SNOW_LAND : 天气恶劣的时候会下雪。
 * C_RESET_SNOW_LAND : 天气恶劣时会下雨或暴风雨。
 * C_SET_SPEED [分] : 设置现实1秒的时间流速
 * C_SHOW_CLOCK : 显示模拟时钟。
 * C_HIDE_CLOCK : 隐藏模拟时钟。
 * C_SET_TIME_REAL : 将时间获取方法更改为现实时间。
 * C_SET_TIME_VIRTUAL : 将获取时间的方式更改为游戏中的时间。
 * C_SET_RAINY_PERCENT [確率] : 设置降水概率(0-100)。
 * C_INIT_TOTAL_TIME : 初始化累计时间、累计天数。
 *
 * ・模拟时钟图像改变命令
 * 您可以更改模拟时钟的图像文件名(img/pictures)。
 * 不过更改后的效果切换地图后才会生效。
 * C_SET_CLOCK_BASE [文件名] : 更改表盘图像的文件名。
 * C_SET_HOUR_HAND [文件名] : 更改短针图像的文件名。
 * C_SET_MINUTE_HAND [文件名] : 更改长针图像的文件名。
 *
 * ・定时器操作命令
 * 在指定的时间（分钟）经过后，将打开开关或独立开关。
 * 可以和现实时间联动。
 * 开关请指定开关ID，独立开关请指定ABCD
 * 
 *
 ...
 * 
 *
 * C_SET_SWITCH_NAMED_TIMER [定时器名称] [分] [スイッチID] [ループ]
 * 指定例（游戏时间30分钟过后，打开开关10）
 * C_SET_SWITCH_NAMED_TIMER timer 30 10 ON
 *
 * C_SET_SELF_SWITCH_NAMED_TIMER [定时器名称] [分] [セルフスイッチ種類] [ループ]
 * 指定例（游戏时间3小时过后，打开独立开关B）
 ...
 *
 * 这是一项警报功能，可让您通过指定时刻来操作开关。
 * 以[年月時分]は、「YYYYMMDDHHMM」形式指定。
 * 取消计时器使用以下命令。
 * C_SET_SWITCH_ALARM [年月時分] [开关ID] [时间间隔]
 * C_SET_SELF_SWITCH_ALARM [年月時分] [独立开关类型] [时间间隔]
 * C_SET_SWITCH_NAMED_ALARM [警报名称] [年月時分] [开关ID] [时间间隔]
 * C_SET_SELF_SWITCH_NAMED_ALARM [警报名称] [年月時分] [独立开关类型] [时间间隔]
 *
 * 指定例（在游戏时间内，于2019年10月22日15:00后打开独立开关[B]。）
 * C_SET_SELF_SWITCH_ALARM 201910221500 B
 *
 * 如果指定间隔，则开关打开后，经过指定时间后开关会再次打开。
 * 单位为分钟，可以使用计算公式。
 *
 * 指定例（当游戏时间超过2019/10/22 15：00时，独立开关[B]打开。
 * 　　　　之后，每经过一天就再次打开独立开关[B]）
 * C_SET_SELF_SWITCH_ALARM 201910221500 B 24*60
 *
 * 备注栏详细信息
 *  在标题集或备注栏里填入以下内容，可以暂时禁用天气和色调效果。
 *  在事件过程或室内地图里你可能会用到。
 *  设置优先考虑地图的备注栏。
 *  
 *
 ...
 * <C_雪:ON>       # 同上
 *
 * 你可以为事件设置执行时，时间是否正常流逝。
 * 设置优先于插件设置。
 * 在事件的备注栏里填入以下内容。
 * <C_時間経過:ON> # 事件执行时时间流逝。(ON/OFF)
 * <C_NoStop:ON>   # 同上
 *
 * 高级设置
 * 通过引用源代码中的“用户重写区域”，可以更改以下内容。
 *  时区信息(早上从几点到几点等)
 *  每个时区的色调(但是，如果天气不好，将进行校正)
 *
 * 脚本
 *  将帧数指定为自然时间相加间隔。
