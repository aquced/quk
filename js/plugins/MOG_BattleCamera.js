...
　　var Moghunter = Moghunter || {}; 
	...
	Moghunter.sex_ofsy = Number(Moghunter.parameters['Sex OffsetY'] || 50);
...
Game_Temp.prototype.clearBattleCamera = function() {
	...
	this._bcam_target_turn = [null,[0,0],null/*追加*/];
	...
	this._bcam_range = 100;
	this._bcam_range_target = 100;
	this._bcam_enemy_wait = 101;
};
...
BattleManager.startAction = function() {
	 ...
	 if (this._subject.isEnemy()) {
		$gameTemp._bcam_target_turn[0] = this._subject;
		$gameTemp._bcam_target_turn[2] = this._targets[0];
		$gameTemp._bcam_enemy_wait = 0;
	 }
	 else
...
Sprite_Battler.prototype.cam_th = function() {
	if (this._spine) {
		var ret = this.y - this._spine.skeleton.data.height * 0.5 * this._spine.scale.y;
		// switch ($gameTemp._battleH) {
		// 	case 0:		ret += Moghunter.sex1_ofsy; break;
		// 	case 1:  	ret += Moghunter.sex2_ofsy; break;
		// }
		if ($gameTemp._battleH) {
			ret += Moghunter.sex_ofsy;
		}
		return ret;
	}
	else if (this._mainSprite) {
		return this.y - (this._mainSprite.height * 0.5);
	}
	else if (this.bitmap) {
		return this.y - (this.bitmap.height * 0.5);
	}
	else {
		return this.y;
	}
...
Spriteset_Battle.prototype.cam_range = function() {
	return this._camera_range / $gameTemp._bcam_range;
};	
...
Spriteset_Battle.prototype.updateBattleback = function() {
    // if (!this._battlebackLocated) {
        ...
        if (Math.floor($gameTemp._bcam_range) != Math.floor($gameTemp._bcam_range_target))
	        $gameTemp._bcam_range = Math.lerp($gameTemp._bcam_range, $gameTemp._bcam_range_target, 0.1);
    // }
};
//==============================
// * locateBattleback
...
Spriteset_Battle.prototype.setBackScaleCam = function(sprite) {
	...
	if (sprite.bitmap.width < width)
		sprite.scale.x = width / sprite.bitmap.width;
	if (sprite.bitmap.height < height)
...
Spriteset_Battle.prototype.centerBattleback = function(sprite) {
	...
	// var cam_range = this.cam_range() - 0.5;
	// console.log(cam_range)
	// sprite.anchor.x = 0.5 - nx * cam_range  / Graphics.boxWidth;
	// sprite.anchor.y = 0.5 + ny * cam_range  / Graphics.boxHeight;
	sprite.anchor.x = 0.5;
	sprite.anchor.y = 0.5;
...
Spriteset_Battle.prototype.update_battle_camera = function() {
	if ($gameTemp._isEffecting) {
		// ++$gameTemp._bcam_enemy_wait;
		// console.log($gameTemp._bcam_enemy_wait, $gameTemp._bcam_target_turn[2])
	}
	else if ($gameTemp._bcam_target_turn[2]) {
		$gameTemp._bcam_target_turn[0] = $gameTemp._bcam_target_turn[2];
		$gameTemp._bcam_target_turn[2] = null;
...
Spriteset_Battle.prototype.cam_move_to = function(value,real_value,speed,type)
{
	if (value == real_value)
		return value;
	if (Math.lerp) {
		value = Math.lerp(value, real_value, 0.05);
	}
	else {
		var dnspeed = 5 + (Math.abs(value - real_value) / speed);
		if (value > real_value) {
			value = Math.max(value - dnspeed, real_value);
		}
		else {
			value = Math.min(value + dnspeed, real_value);
		}
	}
	if (type === 0)
		return Math.min(Math.max(value, this._cam_limit[0]), this._cam_limit[1]);
	else
		return Math.min(Math.max(value, this._cam_limit[2]), this._cam_limit[3]);
};
